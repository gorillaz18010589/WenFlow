package com.wen.flow.ui.test.data;
...
public class DataProvider {
    ...
    private static ArrayList<Integer> getData(){
        ...
        TypedArray bannerImage = MyApplication.getInstance().getResources().obtainTypedArray(R.array.image_girls);
        for (int i = 0; i < 30 ; i++) {
    ...
    private static ArrayList<Integer> getData2(){
        ArrayList<Integer> data = new ArrayList<>();
        TypedArray bannerImage = MyApplication.getInstance().getResources().obtainTypedArray(R.array.data_narrow_Image);
        for (int i = 0; i < 30 ; i++) {
            int image = bannerImage.getResourceId(i, R.drawable.girl2);
            data.add(image);
        }
    ...
    public static final int[] PRODUCT_SHOE_COLOR_TYPE_IMAGE = {
            R.drawable.photp_nike_low,
            R.drawable.photp_nike_low,
            R.drawable.photp_nike_low,
            R.drawable.photp_nike_low,
            R.drawable.photp_nike_low,
    ...
    public static ArrayList<Integer> getShoeTypeColor(int page){
        ArrayList<Integer> arrayList = new ArrayList<>();
        if (page == 4) {
            return arrayList;
        }
        for (int i = 0; i < PRODUCT_SHOE_COLOR_TYPE_IMAGE.length; i++) {
            arrayList.add(PRODUCT_SHOE_COLOR_TYPE_IMAGE[i]);
        }
        return arrayList;
    }
